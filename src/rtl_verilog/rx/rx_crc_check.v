...
module rx_crc_check #(
  ...
  function [11:0] count_lanes;
    input [KEEP_WIDTH-1:0] keep;
    integer lane;
    begin
      count_lanes = 12'd0;
      for (lane = 0; lane < KEEP_WIDTH; lane = lane + 1)
      if (keep[lane]) count_lanes = count_lanes + 12'd1;
    end
  endfunction
      ...
      if (in_valid && in_ready) begin
        ...
        if (in_eop) begin
          if (in_fcs_present) begin
            crc_good  <= !in_error && (next_total >= 12'd4) && (next_crc == 32'hDEBB20E3);
            crc_error <= in_error || (next_total < 12'd4) || (next_crc != 32'hDEBB20E3);
          end else begin
            crc_good  <= !in_error;
            crc_error <= in_error;
          end
