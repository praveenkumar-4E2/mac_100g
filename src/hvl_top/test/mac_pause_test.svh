...
task mac_pause_frame_sequence_c::body();
  ...
  foreach (frame_h.payload[i]) if (i > 3) frame_h.payload[i] = 8'h00;  // pad to min frame
...
task mac_pause_rx_test_c::run_stimulus(uvm_phase phase);
  ...
  if (status_data[0])
    `uvm_info("MAC_PAUSE_RX", $sformatf(
              "PASS: pause_active asserted (REG_RX_STATUS=%h)", status_data), UVM_NONE)
  else
    `uvm_error("MAC_PAUSE_RX", $sformatf(
               "FAIL: pause_active not asserted (REG_RX_STATUS=%h)", status_data))

  axi_rx_cnt = env_h.axi4_stream_agent_top_h.passive_agents[0].monitor_h.mon_rcvd_xtn_cnt;
  if (axi_rx_cnt == 0)
    `uvm_info("MAC_PAUSE_RX", "PASS: PAUSE control frame consumed by RX MAC, no client delivery",
              UVM_NONE)
  else
    `uvm_error("MAC_PAUSE_RX", $sformatf(
...
task mac_pause_tx_test_c::run_stimulus(uvm_phase phase);
  ...
  mac_wait_utils_c::wait_for_count_at_least(
      1, env_h.mac_rs_stream_agent_top_h.passive_agents[0].monitor_h.mon_rcvd_xtn_cnt, mac_tx_vif,
      MAC_COMPLETION_TIMEOUT_NS, timed_out);
  if (timed_out)
    `uvm_error("MAC_PAUSE_TX", "timed out waiting for DUT-generated PAUSE frame on MAC TX wire")
  else
    `uvm_info("MAC_PAUSE_TX", "PASS: DUT-generated PAUSE frame observed on MAC TX wire", UVM_NONE)
endtask
