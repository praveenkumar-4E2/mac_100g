 ...
 *    + `tkeep`/`eop_pos` valid-lane counts versus flat byte-array length;
...
`define MAC_FRAME_CODEC_SVH
  ...
  class mac_frame_codec_c;
    ...
    extern static function int axi_to_frame(
        ...
        input bit [7:0] tuser,
        input int unsigned eop_byte_count,
        input mac_frame_dir_e direction,
        input int unsigned eth_len_bound,
    ...
    // eop_byte_count is the valid byte count of the final beat (eop_pos);
    ...
    extern static function int frame_to_axi(
        mac_frame_c frame,
        output byte unsigned axi_q[$]);
    ...
    extern static function int frame_to_rs(
        mac_frame_c frame,
        output byte unsigned wire_q[$]);
  endclass
  ...
  function int mac_frame_codec_c::axi_to_frame(
    ...
    foreach (frame.payload[i])
      frame.payload[i] = frame_q[RS_HDR_BYTES + i];
    ...
    if (fcs_present) begin
      ...
      for (int i = 0; i < RS_FCS_BYTES; i++)
        work.push_back(frame_q[nbytes - RS_FCS_BYTES + i]);
    ...
    frame.result          = error_flag ? MAC_FRAME_RESULT_CRC_ERROR
                                       : MAC_FRAME_RESULT_CLEAN;
    frame.sop             = 1'b1;
    frame.eop             = 1'b1;
  ...
  function int mac_frame_codec_c::rs_to_frame(
    ...
    work = wire_q;
    for (int i = 0; i < RS_PREAMBLE_SFD_BYTES; i++)
      void'(work.pop_front());
    void'(mac_hvl_utils_c::decode_be48(work, frame.da));
    void'(mac_hvl_utils_c::decode_be48(work, frame.sa));
    void'(mac_hvl_utils_c::decode_be16(work, frame.ether_type));
    ...
    foreach (frame.payload[i])
      frame.payload[i] = wire_q[RS_MIN_FRAME_BYTES + i];
    ...
    if (last_fcs) begin
      work.delete();
      for (int i = 0; i < RS_FCS_BYTES; i++)
        work.push_back(wire_q[n - RS_FCS_BYTES + i]);
      void'(mac_hvl_utils_c::wire_bytes_to_fcs(work, frame.fcs));
    end else begin
      frame.fcs = '0;
    end
    ...
    foreach (frame.payload[i])
      work.push_back(frame.payload[i]);
    ...
    frame.result          = crc_error    ? MAC_FRAME_RESULT_CRC_ERROR    :
                            ...
                            last_err     ? MAC_FRAME_RESULT_ALIGNMENT_ERROR :
                                          MAC_FRAME_RESULT_CLEAN;
    frame.sop             = 1'b1;
    frame.eop             = 1'b1;
    frame.eop_byte_count  = eop_byte_count;
  ...
  function int mac_frame_codec_c::frame_to_axi(
      mac_frame_c frame,
      output byte unsigned axi_q[$]);
    if (frame == null)
      return -1;
    ...
    foreach (frame.payload[i])
      axi_q.push_back(frame.payload[i]);
    // Client-supplied FCS (fcs_present) is emitted LSB-first.
    if (frame.fcs_present)
      void'(mac_hvl_utils_c::fcs_to_wire_bytes(axi_q, frame.fcs));
    return 0;
  endfunction

  function int mac_frame_codec_c::frame_to_rs(
      mac_frame_c frame,
      output byte unsigned wire_q[$]);
    if (frame == null)
      return -1;
    ...
    foreach (frame.payload[i])
      wire_q.push_back(frame.payload[i]);
    // Client-supplied FCS (fcs_present) is emitted LSB-first.
    if (frame.fcs_present)
      void'(mac_hvl_utils_c::fcs_to_wire_bytes(wire_q, frame.fcs));
    return 0;
  endfunction
