...
function void mac_frame_c::do_copy(uvm_object rhs);
  ...
  if (!$cast(rhs_h, rhs)) begin
    `uvm_fatal("TYPE_MISMATCH",
               $sformatf("do_copy: %s is not a mac_frame_c", rhs.get_type_name()))
  end
  super.do_copy(rhs);
  da              = rhs_h.da;
  sa              = rhs_h.sa;
...
function string mac_frame_c::convert2string();
  return {$sformatf("DIR=%s RES=%s DA=%h SA=%h ET=%h FCS=%h FCS_ON=%0b ",
                    direction.name(), result.name(), da, sa, ether_type, fcs,
                    fcs_present),
          $sformatf("PLEN=%0d SOP=%0b EOP=%0b EOP_BYTES=%0d PRE=%0b SFD=%h",
                    payload.size(), sop, eop, eop_byte_count,
                    preamble_present, sfd)};
endfunction
