 ...
 *  - clock/event-based sampling when a clock source (`virtual mac_if`) is
...
`define MAC_WAIT_UTILS_SVH
  ...
  class mac_wait_utils_c;
    ...
    //------------------------------------------------------------------------
    static task wait_for_config_done(
        mac_tb_cfg_c cfg_h,
    ...
    // Sampling is clock/event-based on the supplied mac_if clock edge when
    ...
    //------------------------------------------------------------------------
    static task wait_for_count_at_least(
        input int unsigned     target,
        ref int                count,
        input virtual mac_if   vif,
        input time             timeout_ns,
        output bit             timed_out
    );
      time start_ts;
    ...
    //------------------------------------------------------------------------
    static task wait_for_queue_empty(
        ref byte unsigned q[$],
        input virtual mac_if vif,
        input time           timeout_ns,
        output bit           timed_out
