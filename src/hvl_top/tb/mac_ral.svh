...
function void mac_apb_reg_adapter_c::bus2reg(uvm_sequence_item bus_item,
                                              ref uvm_reg_bus_op rw);
  apb_transfer_t response_h;
  if (!$cast(response_h, bus_item)) begin
    rw.status = UVM_NOT_OK;
    `uvm_error("MAC_RAL_ADAPTER",
               $sformatf("Expected apb_transfer_t, got %s", bus_item.get_type_name()))
    return;
  end
...
class mac_ral_reg_c extends uvm_reg;
  ...
  extern function void configure_metadata(string access,
...
function void mac_ral_reg_c::configure_metadata(string access,
                                                 uvm_reg_data_t reset = '0,
                                                 bit volatile_value = 0);
