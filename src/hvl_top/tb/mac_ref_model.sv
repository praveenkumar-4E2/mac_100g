...
function void mac_reference_model_c::write_rs(frame_xtn_c m_rs_xtn);
  ...
  if (m_rs_xtn.crc_error || m_rs_xtn.length_error || m_rs_xtn.alignment_error || is_pause_frame(
          m_rs_xtn
      ))
    return;
...
function void mac_reference_model_c::write_apb_ref(apb_transfer_t m_apb_xtn);
  ...
  case (m_apb_xtn.addr)
    ...
    reg_map_pkg::REG_PAUSE_TX_CONFIG: begin
      cfg_pause_tx_enable = m_apb_xtn.wdata[reg_map_pkg::PAUSE_TX_ENABLE_BIT];
      ...
      cfg_pause_tx_soft_req = m_apb_xtn.wdata[reg_map_pkg::PAUSE_TX_SOFT_REQ_BIT];
    end
    default: ;
  endcase
endfunction
