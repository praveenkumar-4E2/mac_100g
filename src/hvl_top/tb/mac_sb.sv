...
function void mac_scoreboard_c::compare_tx(frame_xtn_c expected, frame_xtn_c actual);
  ...
  if (cfg_h.scoreboard_check_ether_type)
    match &= (expected.ether_type === actual.ether_type);
  if (cfg_h.scoreboard_check_fcs)
    match &= (expected.fcs === actual.fcs) &&
...
function bit mac_scoreboard_c::has_unmatched();
  return (axi_expected_q.size() != 0 || axi_actual_q.size() != 0 ||
...
function void mac_scoreboard_c::flush_pending(string reason);
  ...
  flushed += axi_expected_q.size() + axi_actual_q.size() +
             rs_expected_q.size() + rs_actual_q.size();
  axi_expected_q.delete(); axi_actual_q.delete();
  rs_expected_q.delete();  rs_actual_q.delete();
  if (flushed != 0) begin
    reset_flushes += flushed;
    `uvm_info("MAC_SB_RESET", $sformatf("flushed %0d pending frame(s): %s", flushed, reason), UVM_MEDIUM)
  end
endfunction
...
function void mac_scoreboard_c::check_unmatched_timeout();
  ...
  if (($time - last_match_time) >= cfg_h.scoreboard_unmatched_timeout_ns) begin
    timed_out++;
    mismatches++;
    `uvm_error("MAC_SB_TIMEOUT", $sformatf("unmatched frame timeout after %0t: tx_exp=%0d tx_act=%0d rx_exp=%0d rx_act=%0d",
                                             cfg_h.scoreboard_unmatched_timeout_ns,
                                             rs_expected_q.size(), rs_actual_q.size(),
...
function void mac_scoreboard_c::report_phase(uvm_phase phase);
  `uvm_info("MAC_SB", $sformatf("scoreboard summary: tx_matches=%0d rx_matches=%0d mismatches=%0d timed_out=%0d reset_epochs=%0d reset_flushed=%0d",
                                 tx_matches, rx_matches, mismatches, timed_out,
                                 reset_epochs, reset_flushes), UVM_NONE)
endfunction
