...
function void apb_agent_cfg_c::validate();
  ...
  if (m_vif == null) begin
    ...
    if (m_addr_width == 0)
      m_addr_width = $bits(m_vif.paddr);
    else if (m_addr_width != $bits(m_vif.paddr))
      problems = {problems, $sformatf(" m_addr_width=%0d != $bits(vif.paddr)=%0d",
                                      m_addr_width, $bits(m_vif.paddr))};
    if (m_data_width == 0)
      m_data_width = $bits(m_vif.pwdata);
    else if (m_data_width != $bits(m_vif.pwdata))
      problems = {problems, $sformatf(" m_data_width=%0d != $bits(vif.pwdata)=%0d",
                                      m_data_width, $bits(m_vif.pwdata))};
  end
  ...
  if (m_max_wait_cycles == 0)
    problems = {problems, $sformatf(" m_max_wait_cycles=%0d unusable (must be >0)",
                                    m_max_wait_cycles)};
  if (m_require_word_alignment) begin
    if (m_address_alignment == 0)
      problems = {problems, $sformatf(" alignment enabled but m_address_alignment=%0d",
                                      m_address_alignment)};
    else if ((m_address_alignment & (m_address_alignment - 1)) != 0)
      problems = {problems, $sformatf(" m_address_alignment=%0d not a power of two",
                                      m_address_alignment)};
  end
...
function string apb_agent_cfg_c::convert2string();
  return {$sformatf("mode=%0s monitor=%0b vif=%0d addr_w=%0d data_w=%0d",
                    m_is_active.name(), m_has_monitor, (m_vif != null),
                    m_addr_width, m_data_width),
           $sformatf(" max_wait=%0d release=%0d align_en=%0b align=%0d known=%0b",
                     m_max_wait_cycles, m_reset_release_cycles,
                     m_require_word_alignment, m_address_alignment,
                     m_check_known_values),
           $sformatf(" logger=%0b verbosity=%0s checks=%0b cov=%0b fail_slverr=%0b",
                     m_enable_logger, m_transfer_verbosity.name(),
                     m_enable_protocol_checks, m_enable_coverage,
