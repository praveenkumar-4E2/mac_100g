...
function void mac_reset_agent_cfg_c::validate();
  ...
  if (default_duration_cycles == 0)
    problems = {problems, " default_duration_cycles=0"};
  if (problems != "")
    `uvm_fatal("RESET_CFG", $sformatf("%s: invalid configuration:%0s",
                                       get_type_name(), problems))
endfunction

function string mac_reset_agent_cfg_c::convert2string();
  return $sformatf("is_active=%0s reset_id=%0d vif=%0d active_level=%0b duration=%0d sync=%0b has_monitor=%0b",
                   is_active.name(), reset_id, (vif != null), active_level,
                   default_duration_cycles, synchronize_to_clock, has_monitor);
endfunction
