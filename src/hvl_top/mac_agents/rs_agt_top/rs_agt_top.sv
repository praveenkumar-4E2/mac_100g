...
function void rs_agent_top_c::build_phase(uvm_phase phase);
  ...
  foreach (active_agents[i]) begin
    ...
    uvm_config_db#(rs_agent_cfg_c)::set(this, $sformatf("active_agents[%0d]*", i),
                                            "rs_agent_cfg", cfg_h.rs_active_agent_cfgs[i]);
  ...
  foreach (passive_agents[i]) begin
    ...
    uvm_config_db#(rs_agent_cfg_c)::set(this, $sformatf("passive_agents[%0d]*", i),
                                            "rs_agent_cfg", cfg_h.rs_passive_agent_cfgs[i]);
