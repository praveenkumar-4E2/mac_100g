...
function void frame_xtn_c::do_copy(uvm_object rhs);
  ...
  if (!$cast(rhs_h, rhs)) begin
    `uvm_fatal("TYPE_MISMATCH",
               $sformatf("do_copy: %s is not a frame_xtn_c", rhs.get_type_name()))
  end
  super.do_copy(rhs);
...
function string frame_xtn_c::convert2string();
  return {$sformatf("PRE=%h SFD=%h DA=%h SA=%h ET=%h FCS=%h PLEN=%0d FCS_ON=%0b ",
                    preamble, sfd, dst_addr, src_addr, ether_type, fcs,
                    payload.size(), insert_fcs),
